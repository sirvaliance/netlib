 ...
 *   3) resolve a name1 (or address1) again, result comes from the cache (cannot come from 2nd mock instance)
 *   4) wait until time-to-live end, i.e. cache is empty then
 *   5) resolve a name1 (or address1) again, this fails now because it cannot come from 2nd mock instance)
 *   6) resolve a name2 (or address2), result comes from 2nd mock instance
 *   
 * Setup: SwitchingNetAddressNameService(2xMock) + on-top CachingNetAddressNameService:
 * <pre>
 *   +-------------------------------------------------+
...
public class SwitchingAndCachingNetAddressNameServiceLocalTest {
    private static final Logger log = Logger.getLogger(SwitchingAndCachingNetAddressNameServiceLocalTest.class.getName());
    
    // the following host name - IP combinations should be different to real live mappings
    private static final String TEST_HOSTNAME1 = "dnstest.silvertunnel.org";
    ...
    /** SwitchingNetAddressNameService(2xMock) + on-top CachingNetAddressNameService */
    ...
    public void testGetAddressesByName() throws Exception {
        ...
        // 1a) resolve a name2 (or address2), negative result comes from 1st mock instance
        try {
        ...
        // 3) resolve a name1 (or address1) again, result comes from the positive cache (cannot come from 2nd mock instance)
        resolvedIps = ns.getAddressesByName(TEST_HOSTNAME1);
        assertEquals("wrong number of IPs found", 1, resolvedIps.length);
        assertEquals("wrong IP found", TEST_IP1, resolvedIps[0]);

        // 3a) resolve a name2 (or address2), negative comes from the positive cache (cannot come from 2nd mock instance)
        try {
            resolvedIps = ns.getAddressesByName(TEST_HOSTNAME2);
            // this should not be executed:
            String resolvedIpsStr = (resolvedIps==null) ? null : Arrays.toString(resolvedIps);
        ...
        // 4) wait until time-to-live end, i.e. cache is empty then
        Thread.sleep((1000L*(Math.max(CACHE_TTL_SECONDS, CACHE_NEGATIVE_TTL_SECONDS)))+100L);
    ...
    public void testGetNamesByAddress() throws Exception {
        ...
        try {
            ...
            // this should not be executed:
            String resolvedNamesStr = (resolvedNames==null) ? null : Arrays.toString(resolvedNames);
            fail("expected UnknownHostException not thrown, resolvedIps="+resolvedNamesStr);
        } catch (UnknownHostException e) {
            // expected
        }
