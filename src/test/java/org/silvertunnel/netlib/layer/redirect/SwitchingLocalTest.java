...
public class SwitchingLocalTest {
    private static final Logger log = Logger.getLogger(SwitchingLocalTest.class.getName());

    private byte[] USER_DATA_REQUEST;
    
    // 1st lower layer/1st request
    private byte[] USER_DATA_RESPONSE1;
    private NetAddress remoteNetAddressOfRequest1 = new TcpipNetAddress("host1:1001");

    // 2nd lower layer/2nd request
    private byte[] USER_DATA_RESPONSE2;
    private NetAddress remoteNetAddressOfRequest2 = new TcpipNetAddress("host1:1001");
    ...
    public SwitchingLocalTest() {
        try {
            ...
            USER_DATA_RESPONSE1 = ByteArrayUtil.getByteArray("Hier ist\n\nmeine erste Antwort\n fuer heute", 3001, "\u00e0.");
            USER_DATA_RESPONSE2 = ByteArrayUtil.getByteArray("Hier ist\n\nmeine zweite Antwort\n fuer heute", 3002, "\u00e0.");
     ...
     * Then switch the lower layer.
     * Then create and test a connection (expected to be with 2nd lower layer).
     * 
     * @throws Exception
     */
    @Test(timeout=2000)
    public void testSocketSwitchBetweenTwoConnections() throws Exception {
        // create and test a connection (expected to be with 1st lower layer)
        NetSocket netSocket = switchingNetLayer.createNetSocket(null, null, remoteNetAddressOfRequest1);
        requestResponseHelp(netSocket, remoteNetAddressOfRequest1, USER_DATA_REQUEST, USER_DATA_RESPONSE1,  lowerNetLayer1);
        ...
        netSocket = switchingNetLayer.createNetSocket(null, null, remoteNetAddressOfRequest2);
        requestResponseHelp(netSocket, remoteNetAddressOfRequest2, USER_DATA_REQUEST, USER_DATA_RESPONSE2,  lowerNetLayer2);
    }
    
    /**
     * Create 1st connection (expected to be with 1st lower layer).
     * Then switch the lower layer with closeAllOpenConnectionsImmediately=true
     * Then use and test 1st connection (expected to fail)
     * Then create and test a connection (expected to be with 2nd lower layer).
     * 
     * @throws Exception
     */
    @Test(timeout=2000)
    public void testSocketSwitchDuringConnection_with_closeAllOpenConnectionsImmediately() throws Exception {
        // create 1st connection (expected to be with 1st lower layer)
        NetSocket netSocket = switchingNetLayer.createNetSocket(null, null, remoteNetAddressOfRequest1);
        ...
        try {
            ...
            // expected because: netSocket should be closed after switch with  closeAllOpenConnectionsImmediately=true
        }
    ...
    /**
     * Create 1st connection (expected to be with 1st lower layer).
     * Then switch the lower layer with closeAllOpenConnectionsImmediately=false
     * Then use and test 1st connection (expected to be with 1st lower layer)
     * Then create and test a connection (expected to be with 2nd lower layer).
     * 
     * @throws Exception
     */
    @Test(timeout=2000)
    public void testSocketSwitchDuringConnection_without_closeAllOpenConnectionsImmediately() throws Exception {
        // create 1st connection (expected to be with 1st lower layer)
        NetSocket netSocket = switchingNetLayer.createNetSocket(null, null, remoteNetAddressOfRequest1);
     ...
     * @throws Exception
     */
    protected void requestResponseHelp(NetSocket netSocket, NetAddress remoteNetAddressOfRequest, byte[] userDataRequest,
            byte[] expectedUserDataResponse, MockNetLayer expectedMockNetLayer) throws Exception {
        ...
        ByteArrayTestUtil.assertByteArrayFromInputStream(null, "wrong user data response", expectedUserDataResponse, netSocket.getInputStream()); 
        
        // check user data received by the remote side (mock)
        TestUtil.waitUntilMinimumNumberOfReceivedBytes(expectedMockNetLayer.getFirstSessionHistory(), userDataRequest.length);
        netSocket.close();
        NetAddress expectedNetAddress = remoteNetAddressOfRequest;
        TestUtil.assertMockNetLayerSavedData("wrong data received by mock", expectedMockNetLayer.getFirstSessionHistory(), userDataRequest, expectedNetAddress);
    }
}
